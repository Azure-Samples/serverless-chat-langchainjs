...
In this tutorial, we will create a function that uses Azure OpenAI to answer questions. To do this, we will use the `@langchain/azure-openai`
...
// TODO: explicar o que é cada variável
...
3. We instantiate the `AzureOpenAIEmbeddings` class passing the access credentials to Azure OpenAI.
...
**todo: explicar melhor o que é uma representação vetorial**

No próximo passo, estaremos iniciando a criação do uso do `CosmosDB LC vector store` para armazenar os vetores gerados pelo Azure OpenAI.
